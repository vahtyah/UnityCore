namespace VahTyah
{
    ...
    public static class Haptic
    {
        public static void Play(HapticType type, bool force = false)
            => EventBus.Publish(new HapticPlay { Type = type, Force = force });

        public static void PlaySequence(bool force, params HapticType[] types)
            => EventBus.Publish(new HapticSequence { Types = types, Force = force });

        public static void SetActive(bool active)
            => EventBus.Publish(new HapticSetActive { Active = active });
    }
}
